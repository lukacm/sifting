...
int read_input_file(char **argv,int &input_counter, int *inout, int **&inputcubes, int **&outputcubes, int **&inputcubes_for_process, int **&outputcubes_for_process, int *result){
	string line;
if (argv[2] == 0){
...
//	cout<<"Inout23: "<< inout[0]<< " Input counter: "<<input_counter<<" Input Cubes: "<<inputcubes<<endl;
	if (argv[2][0] == '0') {
		...
		in_stream.clear();              // forget we hit the end of file
		in_stream.seekg(0, ios::beg);   // move to the start of the file
		read_real_file(input_counter, inout, incubes, outcubes);
